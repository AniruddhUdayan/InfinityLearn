	...
	return (
		<div className="faculty-wrapper">
			<div className="faculty-top">
				<span>study with india's <span className='blue'>best {examFor?.toUpperCase() ?? 'JEE'} teachers</span></span>
				<div className="faculty-btns">
			...
			<div className='faculty-progress'>
				<div>01</div>
				<LinearProgress variant="determinate" value={normalise(progress+1)} className='faculty-progress-actual' />
				<div>{faculties?.length}</div>
			</div>
			<div ref={carousel} className="faculty-cards no-scrollbar">
				{faculties.map((f, i) => (
					<FacultyCard ref2={carouselEle} key={i} name={f?.name} image={f?.image} headline={f?.headline} subject={f?.subject} usp={f?.usp} />
				))}
			</div>
			<div className="faculty-explore ">
				explore all
			</div>
		</div>
	)
